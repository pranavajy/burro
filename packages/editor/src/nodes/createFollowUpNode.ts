...
export function createFollowUpNode(
	...
	window.setTimeout(() => {
		...
		// Bring the composer into view without making a single draft card fill
		// the screen. Preserve an already zoomed-out canvas and cap closer views.
		editor.zoomToBounds(nodeBounds, {
			targetZoom: Math.min(editor.getZoomLevel(), 0.85),
